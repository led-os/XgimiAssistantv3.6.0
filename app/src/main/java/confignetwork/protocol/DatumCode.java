...
import confignetwork.task.ICodeData;
import confignetwork.util.ByteUtil;
import confignetwork.util.CRC8;
import confignetwork.util.NetUtil;


public class DatumCode implements ICodeData {
	
	// define by the Ch protocol, all of the datum code should add 1 at last to prevent 0
	...
	public DatumCode(String apSsid, String apBssid, String apPassword, InetAddress ipAddress, boolean isSsidHiden) {
		...
		crc.reset();
		crc.update(NetUtil.parseBssid2bytes(apBssid));
