 ...
 */
public class SearchJiLuPianFragment extends BaseFragment {
    public SearchJiLuPianFragment() {
    ...
    public View onCreateView(LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        if (view == null) {
            view = inflater.inflate(R.layout.fragment_search_jilupian, container, false);
            initView(view);
        }
        return view;
    ...
    private void initView(View view) {
        mNoResourceTv = (TextView) view.findViewById(R.id.tv_no_resource_search_jilupian);
        adapter = new SearchMovieAdapter(getActivity(), SaveData.getInstance().searchmovie, SaveData.getInstance().jlp);
    ...
    public void onResume() {
        super.onResume();
        if (SaveData.getInstance().searchmovie.get(SaveData.getInstance().jlp).list == null
                || SaveData.getInstance().searchmovie.get(SaveData.getInstance().jlp).list.size() == 0) {
