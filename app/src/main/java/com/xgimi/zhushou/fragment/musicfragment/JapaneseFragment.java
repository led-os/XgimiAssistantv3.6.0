 ...
 */
public class JapaneseFragment extends BaseFragment implements View.OnClickListener,SwipeRefreshLayout.OnRefreshListener{
    public View view;
...
//        initLocal();
    ...
    private void initExtra() {
        ...
        if(SaveData.getInstance().mTypes!=null) {
            order=SaveData.getInstance().mTypes.data.get(6).order;
        }
    }
        ...
        net_connect.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                if(HttpUrl.isNetworkConnected(getActivity())){
                    ...
                    if(SaveData.getInstance().mTypes!=null) {
                        getMV(page, psize, SaveData.getInstance().mTypes.data.get(6).area_id, user_id,order);
                    }
                }
            }
        });
    ...
    private void initLocal() {
        String readHomeJson = app.readHomeJson("Japanese");
    ...
    private void initData() {
        ...
        recyclerView.addOnScrollListener(new EndLessOnScrollListener(false,mLayoutManager) {
            ...
            public void onLoadMore(int currentPage) {
                if(SaveData.getInstance().mTypes!=null) {
                    getMV(currentPage, psize, SaveData.getInstance().mTypes.data.get(6).area_id, user_id,order);
                    isRefresh=true;
                }
            }
        });
        ...
        public void onNext(MVList mvList) {
            if(mvList!=null&&mvList.data!=null){
                if(mpage==1){
                    ...
                    String Japanese=new Gson().toJson(mvList);
                    app.savaHomeJson("Japanese",Japanese);
                }
                loadHome(mvList);
            }else {
                recyclerView.setVisibility(View.GONE);
