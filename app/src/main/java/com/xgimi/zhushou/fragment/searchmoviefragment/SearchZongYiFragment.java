 ...
 */
public class SearchZongYiFragment extends BaseFragment{

    public SearchZongYiFragment(){
    ...
    public View onCreateView(LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        if (view == null) {
            view=inflater.inflate(R.layout.fragment_search_zongyi,container,false);
            initView(view);
        }
        return view;
    }
    ...
    private void initView(View view) {
        mNoResourceTv = (TextView) view.findViewById(R.id.tv_no_resource_search_zongyi);
        adapter=new SearchMovieAdapter(getActivity(), SaveData.getInstance().searchmovie,SaveData.getInstance().zy);
    ...
    public void onResume() {
        super.onResume();
        if (SaveData.getInstance().searchmovie.get(SaveData.getInstance().zy).list == null
                || SaveData.getInstance().searchmovie.get(SaveData.getInstance().zy).list.size() == 0) {
