 ...
 */
public class ApplyYingYongAdapter extends BaseAdapter {
    ...
    public ApplyYingYongAdapter(Context context, ApplySearc data) {
        this.mContext = context;
        this.mData = data;
//        for (int i = 0; i < SaveTVApp.getInstance(mContext).mTvApp.appList.size(); i++) {
//            XGIMILOG.D("" +  SaveTVApp.getInstance(mContext).mTvApp.appList.get(i).packageName);
//        }
    }
...
//        XGIMILOG.D("ietm : " + mData.data.get(position).package_name + ", list.size = " + SaveTVApp.getInstance(mContext).mTvApp.appList.size());
...
//                        vh.open.setText("排队中");
                        ...
                        Log.e("laile", myInfo.downloadInfo.progress + "%");
                    } else {
            ...
            public void onClick(View v) {
                if (Constant.netStatus) {
                    if (ToosUtil.getInstance().isInstallTvControl((Activity) mContext)) {
                        ...
                        if (vh.open.getText().toString().trim().equals("打开应用")) {
...
//                            mTv = vh.open;
                            ...
                            mPlays.add(playList);
                            ToosUtil.getInstance().addEventUmeng(mContext, "event_app_install");
...
//                       mTv.setText("排队中");
                ...
                if (info.installInfo != null && info.installInfo.stat == 1) {
                    ...
                    notifyDataSetChanged();
//                                    mTVMaps.remove(info.downloadInfo.filename);
                }
