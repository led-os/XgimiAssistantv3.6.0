...
public class CircleImageView extends ImageView {
	...
	private static final int DEFAULT_ROUND_WIDTH = 0;
	...
	private float mRoundWidth = 30; // 矩形的圆角大�?
	...
	public CircleImageView(Context context, AttributeSet attrs, int defStyle) {
		...
		// // 圆角半径.
		// mRoundWidth = a.getDimensionPixelSize(
		// R.styleable.CircleImageView_round_width, DEFAULT_ROUND_WIDTH);
		// a.recycle();
	...
	protected void onDraw(Canvas canvas) {
		...
		if (mRoundWidth != 0) { // 矩形圆弧.
			canvas.drawRoundRect(new RectF(getLeft(), getTop(), getRight(),
					getBottom()), mRoundWidth, mRoundWidth, mBitmapPaint);
			if (mBorderWidth != 0) {
				canvas.drawRoundRect(new RectF(getLeft(), getTop(), getRight(),
						getBottom()), mRoundWidth, mRoundWidth, mBorderPaint);
			}
		} else { // 圆形.
